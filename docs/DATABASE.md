...
| Column         | Type                                                | Description                                  |
| -------------- | --------------------------------------------------- | -------------------------------------------- |
| id             | BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY | Unique token ID                              |
| tokenable_id   | BIGINT                                              | Polymorphic relation to users                |
| tokenable_type | VARCHAR(255)                                        | Polymorphic type (usually `App\Models\User`) |
| name           | TEXT                                                | Token name (e.g., dev-token)                 |
| token          | VARCHAR(64) UNIQUE                                  | Hashed token value                           |
| abilities      | TEXT                                                | JSON/array of abilities                      |
| last_used_at   | TIMESTAMP NULL                                      | Last usage of token                          |
| expires_at     | TIMESTAMP NULL                                      | Optional expiration datetime                 |
| created_at     | TIMESTAMP DEFAULT CURRENT_TIMESTAMP                 | When the token was created                   |
| updated_at     | TIMESTAMP DEFAULT CURRENT_TIMESTAMP                 | When the token was last updated              |
